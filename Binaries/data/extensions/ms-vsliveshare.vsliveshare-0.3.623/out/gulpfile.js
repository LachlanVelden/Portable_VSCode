...
function createDownloadHash(runtimeDependencies) {
    return __awaiter(this, void 0, void 0, function* () {
        const fileHashMap = {};
        yield Promise.all(runtimeDependencies.map((dep) => __awaiter(this, void 0, void 0, function* () {
            console.log(`Downloading ${dep.url}...`);
            const data = yield download(dep.url, null, {
                followRedirect: true
            });
            ...
            const fileNames = yield getAllFilesAsync(tempDir);
            ...
            yield Promise.all(fileNames.map((fileName) => __awaiter(this, void 0, void 0, function* () {
                ...
                const hash = yield createHash(path.join(tempDir, fileName));
                const normalizedFileName = fileName.replace(/\\/g, '/');
                fileHashMap[dep.code][normalizedFileName] = hash;
            })));
            yield deletePath(tempDir);
        })));
        return fileHashMap;
    });
}
...
gulp.task('external-manifest', function () {
    return __awaiter(this, void 0, void 0, function* () {
        const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, 'package.json')));
        const { runtimeDependencies } = pkg;
        const downloadHash = yield createDownloadHash(runtimeDependencies);
