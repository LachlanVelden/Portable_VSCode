...
class DeprecatedWorkspaceProvider {
    ...
    onFilesChanged(e) {
        const changes = e.changes.map(change => {
            const changePath = (config.featureFlags.multiRootWorkspaceVSCode) ? change.fullPath : change.path;
            const fileChange = {
                type: DeprecatedWorkspaceProvider.toFileChangeType(change.changeType),
                resource: vscode.Uri.parse(config.get(config.Key.scheme) + ':/' + changePath)
            };
            return fileChange;
        });
