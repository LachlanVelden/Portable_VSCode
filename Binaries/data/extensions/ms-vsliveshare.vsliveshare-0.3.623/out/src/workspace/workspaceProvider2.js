...
class WorkspaceProvider {
    ...
    onFilesChanged(e) {
        const changes = e.changes.map(change => {
            ...
            const fileChange = {
                ...
                uri: vscode.Uri.parse(config.get(config.Key.scheme) + ':/' + changePath)
            };
            return fileChange;
        });
        this.onFilesChangedEmitter.fire(changes);
    ...
    stat(uri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const fileInfos = yield this.fileService.listAsync(paths, fileListOptions);
            // Invoke onStat method on providers
            for (const provider of this.workspaceProviders.filter(p => p.onStat)) {
                yield provider.onStat(fileInfos);
            }
    ...
    readFile(uri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const provider of this.workspaceProviders.filter(p => p.onReadFile)) {
                yield provider.onReadFile(fileTextInfo);
            }
