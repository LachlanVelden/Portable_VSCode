...
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (workspaceId) {
            yield context.workspaceState.update(commands_1.Commands.joinWorkspaceIdSettingName, workspaceId);
        }
        if (workspaceFolder) {
            yield context.workspaceState.update(commands_1.Commands.joinWorkspaceIdFolderSettingName, workspaceFolder);
        }
        if (!workspaceId && workspaceStorageId && joinUtilities_1.JoinUtilities.isBrokenLiveshareWorkspaceFile(vscode.workspace)) {
            yield joinUtilities_1.JoinUtilities.restoreLiveshareWorkspaceState(workspaceStorageId);
        }
        ...
        const currentAlternateFlag = context.globalState.get(statusbar_1.ALTERNATE_SHARE_EXPERIMENT_NAME);
        if (currentAlternateFlag !== undefined) {
            telemetry_1.Instance.addContextProperty(telemetryStrings_1.TelemetryPropertyNames.IS_ALTERNATE_SHARE_BUTTON_TEXT, currentAlternateFlag);
        }
...
function activateInternal(context, activationEvent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const isJoined = typeof joinCorrelationId === 'number';
        if (isJoined) {
        ...
        if (installationResult === downloader_1.EnsureRuntimeDependenciesResult.Failure) {
            ...
            // check if we have already set up the flag for the user
            const currentAlternateFlag = context.globalState.get(statusbar_1.ALTERNATE_SHARE_EXPERIMENT_NAME);
            // Give the alternate share button label to 5% of the new users,
            // if they dont have the flag set yet
            const isTeamStatusSet = !!config.get(config.Key.teamStatus);
            const isInternal = config.get(config.Key.isInternal) || isTeamStatusSet;
            if (currentAlternateFlag === undefined && !isInternal) {
                const isSelected = (Math.random() <= 0.05);
                context.globalState.update(statusbar_1.ALTERNATE_SHARE_EXPERIMENT_NAME, isSelected);
                telemetry_1.Instance.addContextProperty(telemetryStrings_1.TelemetryPropertyNames.IS_ALTERNATE_SHARE_BUTTON_TEXT, isSelected);
            }
        }
        yield util_1.ExtensionUtil.updateExecutablePermissionsAsync();
        yield launcher_1.Launcher.setup(false, !(yield downloader_1.installFileExistsAsync()));
        ...
        const hostAdapterService = new hostAdapterService_1.HostAdapterService(rpcClient, Dependencies_1.dependencies.workspaceAccessControlManager);
        ...
        if (!isJoined) {
            shareDebugManager = new shareDebugManager_1.ShareDebugManager(Dependencies_1.dependencies.rpcClient(), Dependencies_1.dependencies.hostAdapterService(), Dependencies_1.dependencies.fileService(), Dependencies_1.dependencies.debugManager(), Dependencies_1.dependencies.workspaceAccessControlManager());
        }
...
const backgroundAgentStartup = (rpcClient, statusBarController) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        ...
        let errorMessage;
        if (yield downloader_1.isInstallCorrupt(traceSource_1.traceSource)) {
...
function checkIfShowWhatsNewInfo() {
    return __awaiter(this, void 0, void 0, function* () {
        const whatsNewUri = config.get(config.Key.whatsNewUri);
        if (whatsNewUri) {
            yield config.save(config.Key.whatsNewUri, '');
            let result = yield vscode.window.showInformationMessage(`VS Live Share updated! ${config.get(config.Key.whatsNewText)}`, `More info`);
            if (result) {
