    ...
    return __awaiter(this, void 0, void 0, function* () {
        if (args.command) { // A Command returned by VS or VS Code
            ...
            if (localCommands.indexOf(command.command) >= 0) { // Run command locally
                ...
                const commandRun = yield lspClient.sendRequest(vscodeLSP.ExecuteCommandRequest.type, params);
                if (typeof commandRun === 'boolean' && !commandRun) { // Older clients may return null rather than a boolean
                    let result = yield vscode.window.showErrorMessage('The host doesn’t allow running this command. If needed, ask them to enable it.', 'More info');
                    if (result) {
