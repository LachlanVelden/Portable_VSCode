...
class Commands {
        ...
        switch (userInfo.accountStatus) {
            ...
            default:
                ...
                try {
                    ...
                    await this.setupCollaboratorCommands();
                }
                catch (e) {
                    let httpException = e;
                ...
                await lspServer.activateAsync(this.workspaceService, this.workspaceAccessControlManager());
                // Create breakpoint manager instance
                await this.createBreakpointManager(true);
                await WorkspaceTaskService.enable(this.rpcClient, this.workspaceService, this.workspaceAccessControlManager());
    ...
    async showFirewallInformationMessage(messageId, showCancelOption) {
        ...
        let result;
        if (showCancelOption) {
            result = await vscode.window.showInformationMessage(util_1.ExtensionUtil.getString(messageId), { modal: true }, getHelp, ok);
        }
        else {
            let getHelpObject = { title: getHelp, isCloseAffordance: false };
            result = await vscode.window.showInformationMessage(util_1.ExtensionUtil.getString(messageId), { modal: true }, getHelpObject, { title: ok, isCloseAffordance: true });
        ...
        if (config.get(config.Key.requestFeedback) && sessionCount % 5 === 0) {
            ...
            switch (qualitativeFeedback) {
                ...
                case apatheticResponse:
                    await config.save(config.Key.requestFeedback, false);
                    break;
                default:
                    break;
            }
    ...
    async joinCollaboration(joinCollaborationLink, options, retryIfUnauthorized = true) {
        ...
        try {
            ...
            if (httpException.code === VSLS_1.ErrorCodes.UnauthorizedHttpStatusCode && retryIfUnauthorized) {
                ...
                await util_1.ExtensionUtil.showErrorAsync('Join failed. ' + e.message);
            }
        }
    }
    ...
    async onExtensionLoadWithLiveShareWorkspace(workspaceId, cancellationToken, progress) {
        ...
        do {
            ...
            try {
                ...
                await this.accessControlManager().joinCollaboration();
        ...
        await this.joinDebugManager.initialize();
    ...
    async leaveCollaboration(options = {}) {
        ...
        await this.safelyExecute('JoinDebugManager', (this.joinDebugManager && this.joinDebugManager.dispose));
        await this.safelyExecute('BreakpointManager', (this.breakpointManager && this.breakpointManager.dispose));
        ...
        await this.safelyExecute('Disable collaborator commands', this.disposeCollaboratorCommands);
            ...
            try {
                ...
                if (os.platform() === util_1.OSPlatform.LINUX && message && message.includes('org.freedesktop.secrets')) {
                    ...
                    if (error.code === VSLS_1.ErrorCodes.KeychainAccessFailed) {
                        ...
                        const result = await vscode.window.showErrorMessage(util_1.ExtensionUtil.getErrorString(VSLS_1.ErrorCodes.KeychainAccessFailed), { modal: true }, moreInfoItem);
    ...
    async debug() {
        ...
        if (debugSessions.length === 1) {
            await util_1.ExtensionUtil.runWithProgress(() => this.joinDebugManager.joinDebugSession(debugSessions[0]), { title: 'Joining Debug Session...' });
        }
        else {
            let items = [];
            ...
            if (selection) {
                ...
                await util_1.ExtensionUtil.runWithProgress(() => this.joinDebugManager.joinDebugSession(debugSession), { title: 'Joining Debug Session...' });
            }
        }
    }
    ...
    async onWorkspaceConnectionStatusChanged(e) {
        ...
        if (session_1.SessionContext.State === sessionTypes_1.SessionState.Joined) {
            ...
            await util_1.ExtensionUtil.showErrorAsync(message, { modal: true });
        }
    }
    ...
    async setupCollaboratorCommands() {
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.pinCommandId, this.pinCommandHandler, this, /* isEditorCommand */ false);
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.pinFromFileTreeExplorerCommandId, this.pinCommandHandler, this, /* isEditorCommand */ false);
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.pinFromActivityBarCommandId, this.pinCommandHandler, this, /* isEditorCommand */ false);
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.unpinCommandId, this.unpinCommandHandler, this, /* isEditorCommand */ true);
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.unpinFromFileTreeExplorerCommandId, this.unpinCommandHandler, this, /* isEditorCommand */ true);
        await util_1.ExtensionUtil.tryRegisterCommand(Commands.unpinFromActivityBarCommandId, this.unpinCommandHandler, this, /* isEditorCommand */ true);
    }
    async pinCommandHandler(args) {
        ...
        if (coEditorCount === 1 && !alwaysShowParticipants) {
            await session_1.SessionContext.coeditingClient.pin(textEditor, coEditors[0]);
        }
        else if (typeof (args) === 'number') {
            await session_1.SessionContext.coeditingClient.pin(null, args);
        }
        else if (args && (typeof (args) === 'object') && args.sessionId) {
            await session_1.SessionContext.coeditingClient.pin(null, args.sessionId);
        }
        else {
