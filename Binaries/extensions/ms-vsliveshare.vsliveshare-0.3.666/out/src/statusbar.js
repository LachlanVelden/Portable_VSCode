...
exports.ALTERNATE_SHARE_EXPERIMENT_NAME = 'liveshare.isAlternateShareLabel';
/**
 * Function to get share button label according to the A/B study user group.
 * @returns Share button label.
 */
function getShareButtonText() {
    const { globalState } = session_1.SessionContext.extensionContext;
    const label = globalState.get(exports.ALTERNATE_SHARE_EXPERIMENT_NAME)
        ? 'Live Share'
        : 'Share';
    return `$(link-external) ${label}`;
}
exports.getShareButtonText = getShareButtonText;
/**
...
class SignedOutState extends StatusBarState {
    async updateUserStatusBarItem(currentUserItem) {
        ...
        currentUserItem.text = getShareButtonText();
...
class SignedInState extends StatusBarState {
    ...
    async updateUserStatusBarItem(currentUserItem) {
        ...
        const isExperimental = (featureSet === config.FeatureSet.experimental);
        const isInternal = config.get(config.Key.isInternal);
        const emojiNamePostfix = (isInternal || isExperimental) ? ' 🎖' : '';
        const internalNamePostfix = isInternal ? ' (Internal User)' : '';
        const experimentalNamePostfix = isExperimental ? ' (Experimental Features)' : '';
        currentUserItem.text = `$(person) ${this.signedInName}${emojiNamePostfix}`;
        currentUserItem.command = commandId;
        currentUserItem.tooltip = `Signed in to ${config.get(config.Key.name)}${this.signedInInfo}${internalNamePostfix || experimentalNamePostfix}`;
        currentUserItem.show();
    }
    async updateCollabStatusBarItem(collaboratorsItem) {
        collaboratorsItem.text = getShareButtonText();
...
class CollaborationState extends SignedInState {
    ...
    async innerUpdateCollabStatusBarItem(collaboratorsItem, textOnly) {
        ...
        if ((state !== sessionTypes_1.SessionState.Joined && state !== sessionTypes_1.SessionState.Shared) || !util_1.ExtensionUtil.shouldStatusBarItemBeVisible()) {
            return;
        }
...
class JoinedState extends CollaborationState {
    ...
    async updateUserStatusBarItem(currentUserItem) {
        ...
        currentUserItem.text = `$(broadcast) ${this.signedInName}${session_1.SessionContext.IsReadOnly ? ' (Read-only)' : ''}`;
        currentUserItem.command = commandId;
