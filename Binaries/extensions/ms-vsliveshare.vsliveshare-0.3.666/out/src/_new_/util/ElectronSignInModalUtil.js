...
class ElectronSignInModalUtil {
    openModalAndWaitForToken(options, loginUri, context) {
        const { cancellationToken, telemetryEvent, commandName } = context;
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                if (cancellationToken) {
                    cancellationToken.onCancellationRequested(cancelSignInModal);
                }
                ...
                sp.on('close', (code) => {
                    ...
                    if (code === 0 && !isSignedIn && !cancellationToken.isCancellationRequested) {
                        if (telemetryEvent) {
