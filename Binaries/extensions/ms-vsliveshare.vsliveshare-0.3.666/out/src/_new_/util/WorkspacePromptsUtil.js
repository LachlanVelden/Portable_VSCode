...
class WorkspacePromptsUtil {
    ...
    async showFirewallInformationMessage(messageId, showCancelOption) {
        ...
        let result;
        if (showCancelOption) {
            result = await this.notificationUtil.showInformationMessage(this.stringUtil.getString(messageId), { modal: true }, getHelp, ok);
        }
        else {
            let getHelpObject = { title: getHelp, isCloseAffordance: false };
            result = await this.notificationUtil.showInformationMessage(this.stringUtil.getString(messageId), { modal: true }, getHelpObject, { title: ok, isCloseAffordance: true });
