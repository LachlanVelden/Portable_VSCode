...
let JoinCommand = JoinCommand_1 = class JoinCommand {
    ...
    async invoke() {
        ...
        // VALIDATION
        const joinCollaborationLink = await this.fetchJoinLink(options);
        ...
        const workspaceInfo = await this.getWorkspaceFromLink(joinCollaborationLink);
        if (!workspaceInfo) {
            return false;
        }
        ...
        const workspaceFolder = await this.getAndValidateWorkspaceFolder(isNewWindow, workspaceInfo);
        ...
        //       a blank workspace.
        const workspaceUri = await this.persistWorkspaceSettings(workspaceFolder, workspaceFilePath, this.telemetryEvent.getCorrelationId(), workspaceInfo);
    ...
    async fetchJoinLink(options) {
        ...
        if (options.promptForCollaborationLink) {
            ...
            joinCollaborationLink = await this.notificationUtil.showInputBox({
    ...
    async getWorkspaceFromLink(joinCollaborationLink) {
    ...
    async getAndValidateWorkspaceFolder(isNewWindow, workspaceInfo) {
    ...
    async persistWorkspaceSettings(workspaceFolder, workspaceFilePath, correlationId, workspaceInfo) {
        ...
        await configCore.save(configCore.Key.joinWorkspaceLocalPath, workspaceFilePath, true, true);
        await configCore.save(configCore.Key.joinEventCorrelationId, correlationId, true, true);
        await configCore.save(configCore.Key.workspaceReloadTime, Date.now(), true);
