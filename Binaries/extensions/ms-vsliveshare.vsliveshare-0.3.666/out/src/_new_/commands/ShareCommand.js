...
let ShareCommand = class ShareCommand {
    constructor(workspaceService, sessionContext, workspaceFirewallUtil, workspaceEnvironmentUtil, workspacePromptsUtil, configUtil, coEditingManager, shareDebugManager, lspServerManager, workspaceTaskManager, workspaceCommandManager, shareBreakpointManager, guestTrackerManager, textSearchManager, workspaceAccessControlManager) {
        ...
        this.postSetupManagers.push({ status: telemetryStrings_1.TelemetryPropertyNames.INIT_TEXT_SEARCH_SERVICE_COMPLETE, instance: textSearchManager });
    }
    async invoke(options, context) {
        let sessionInfo;
        // Start sharing if not shared
        if (this.sessionContext.State !== sessionTypes_1.SessionState.Shared) {
            sessionInfo = await this.shareWorkspace(context.cancellationToken);
    ...
    async shareWorkspace(cancellationToken) {
        ...
        this.postSetupManagers.forEach(async (x) => {
            await x.instance.init();
