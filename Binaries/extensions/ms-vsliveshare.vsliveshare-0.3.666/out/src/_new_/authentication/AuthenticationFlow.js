...
class AuthenticationFlow {
    ...
    async attemptAuthenticationCheckFlow(options, context, suppressException = false) {
        ...
        let error;
        try {
            isAuthenticated = await this.attemptAuthenticationCheckFlowCore(options, cancellationToken);
        }
        catch (e) {
    ...
    async attemptLoginFlowCoreElectron(options, context) {
        ...
        const loginUri = await this.getLoginUri(instanceId, cancellationToken);
        ...
        return await this.authenticationProvider.attemptLogin(userCode, options, cancellationToken);
    }
    ...
    async attemptLoginFlowCoreBrowser(options, context) {
        ...
        const userCode = await this.authenticationProvider.findLoginCode(instanceId, cancellationToken);
        this.sessionContext.transition(session_1.SessionAction.AttemptSignIn);
        // Attempt to login with the returned user code.
        return await this.authenticationProvider.attemptLogin(userCode, options, cancellationToken);
    }
