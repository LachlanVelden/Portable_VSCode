...
async function activateAsync(workspaceService, workspaceAccessControlManager) {
...
function setupHandlers(languageServerProvider, workspaceAccessControlManager) {
    ...
    connection.onRequest(vscodeLSP.CodeActionRequest.type, async (params) => {
        if (workspaceAccessControlManager &&
            !await workspaceAccessControlManager.canPerformOperation(params, codeActionOperation, true /* sendTelemetry */)) {
            return undefined;
        }
        let uri = p2c.asUri(params.textDocument.uri);
