...
class SessionContext extends events_1.EventEmitter {
    constructor() {
        ...
        this.waitingForJoinPromise = { promise: null, resolve: null, completed: false };
        this.waitingForJoinPromise.promise = new Promise((resolve) => {
            this.waitingForJoinPromise.resolve = resolve;
        }).then(() => { this.waitingForJoinPromise.completed = true; });
    }
    get State() {
     ...
     */
    joined() {
        if (this.waitingForJoinPromise.completed) {
            return;
        }
        this.waitingForJoinPromise.resolve();
    }
    /**
    ...
    updateTelemetryContext(newState, previousState) {
        ...
        if (this.IsCollaborating) {
            if (this.workspaceSessionInfo) {
                ...
                telemetry_1.Instance.startSession(isOwner, settings);
                if (isOwner) {
