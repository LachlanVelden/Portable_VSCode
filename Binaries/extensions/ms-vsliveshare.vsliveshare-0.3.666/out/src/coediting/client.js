...
class Client {
    constructor(parameters) {
        ...
        this.accessControlManager = parameters.accessControlManager;
        const nameTagSetting = config.get(config.Key.nameTagVisibility);
        const nameTagSettingValue = decorators_1.NameTagVisibility[nameTagSetting];
        this.nameTagVisibility = nameTagSettingValue || decorators_1.NameTagVisibility.Activity;
    ...
    get isReadOnlyGuest() {
        return this.isExpert && this.accessControlManager().isReadOnly;
    }
    init() {
    ...
    async _onMessage(msg) {
        ...
        switch (message.messageType) {
            ...
            case vsls.MessageType.FileOpenRequest:
                ...
                if (!fileClient) {
                    const document = await vscode.workspace.openTextDocument(this.pathManager.relativePathToLocalPath(fileName));
    ...
    async RespondToSummonsAsync(clientId) {
        ...
        if (config.get(config.Key.focusBehavior) === 'accept') {
            ...
            let result = await vscode.window.showInformationMessage(`${session_1.SessionContext.collaboratorManager.getDisplayName(clientId)} requested you to follow them.`, acceptAction, rejectAction);
    ...
    updateCoEditorScrollPosition(message) {
        ...
        if (transformedMessage === null) {
            ...
            // significantly different in size than the other participent(s)
            return;
        }
    ...
    pin(editor, userId) {
        ...
        this.jumpTo(userId, (editor ? editor.viewColumn : -1));
    ...
    async handleDesync(reason) {
        ...
        await vscode.window.showErrorMessage('You appear to be out of sync. Please rejoin the session.', { modal: true });
